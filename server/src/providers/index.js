 ...
 * @param {string} id
 * @param {{client?: object}} opts `client` is injected by the parity gates (plex-replay) and
 *   by the offline Kavita tests (a stubbed fetch), which is what keeps this seam honest.
 * @throws when the provider is unknown, of an unsupported kind, or NOT CONFIGURED.
 */
export function providerFor(id, { client = null } = {}) {
  ...
  switch (def.kind) {
    case 'plex':
      ...
      return plexProvider({ def, client });
    ...
    case 'kavita': {
      ...
      return kavitaProvider({ def, apiKey, client });
    }
