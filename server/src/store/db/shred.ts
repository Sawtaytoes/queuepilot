    ...
    contents.items.forEach((pair, index) => {
      ...
      if (isSeq(pair.value)) {
        groups.push({
          name,
          position: index,
          // Both nodes carry comments and they are different comments: `alex:  # a note` and
          // the block above `alex:` hang off the KEY, while a comment on the line after
          // `alex:` and before the first entry hangs off the LIST.
