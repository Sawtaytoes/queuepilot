 ...
 */
export const known = (id) => REGISTRY.get(String(id ?? '')) || null;

/** Test seam: forget what we believe is retained on the broker. */
export const _reset = () => REGISTRY.clear();
 ...
 */
export async function announceDevices(pub) {
  try {
    ...
    const next = new Map([[shield.id, shield]]);
    ...
    try {
      ...
      console.log(`[devices] plex.tv enumeration failed: ${e && e.message ? e.message : e}`);
    }
    ...
    for (const [id, device] of next) {
      const payload = { ...device, seen };
      pub(`${T_DEVICES_BASE}/${id}`, payload, { retain: true });
      REGISTRY.set(id, payload);
    }
  } catch (e) {
    console.log(`[devices] announce failed: ${e && e.stack ? e.stack : e}`);
  }
}
