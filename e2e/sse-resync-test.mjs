...
async function readEvents(port, ms) {
  ...
  try {
    while (Date.now() < deadline) {
      ...
      const step = await Promise.race([
        reader.read(),
        new Promise((r) => setTimeout(() => r({ timeout: true }), timeLeft)),
      ]);
...
const stop = (child) => new Promise((r) => { if (!child) return r(); child.once('exit', r); child.kill(); });
...
// --- Layer 1: on-connect replay frame, no broker --------------------------------------- //
{
  ...
  try {
    ...
    ok('with nothing playing the snapshot is null (still delivered)', now && now.now === null);
...
let startFakeMqtt = null;
try {
  ({ startFakeMqtt } = await import('./fake-mqtt.mjs'));
} catch {
...
if (startFakeMqtt) {
  ...
  try {
    ...
    ok('the replayed snapshot carries the retained ratingKey (no publish after connect)',
      now && now.now && Number(now.now.ratingKey) === RETAINED.ratingKey);
  } finally {
    await stop(srv);
