...
//
// Run:  server/node_modules/.bin/tsx e2e/provider-seam-test.ts   (from the repo root; non-zero on failure)
...
// Held in consts as well as on `process.env`: the assertions below read the files back, and
// `process.env.X` is `string | undefined` however recently it was assigned.
const PROVIDERS_PATH = path.join(SCRATCH, 'providers.yaml');
const SECRETS_PATH = path.join(SCRATCH, 'providers.secrets.yaml');
process.env.PROVIDERS_PATH = PROVIDERS_PATH;
process.env.PROVIDERS_SECRETS_PATH = SECRETS_PATH;
...
ok('requireToken throws, names the provider AND how to fix it', () => {
  assert.throws(() => config.requireToken('kavita', 'kavita'), (e: unknown) => {
    const message = errMessage(e);
    assert.match(message, /NOT CONFIGURED/);
    assert.match(message, /kavita/);
    // Actionable: it must say which env var, and which file.
    assert.match(message, /KAVITA_API_KEY/);
    assert.match(message, /providers\.secrets\.yaml/);
    return true;
  });
});
...
await okAsync('writeSecret creates the file 0600', async () => {
  ...
  const mode = statSync(SECRETS_PATH).mode & 0o777;
...
ok('the public view exposes `configured` as a BOOLEAN and no token at all', () => {
  // The fixture defines `kavita`, so the lookup is non-null by construction.
...
ok('the secrets file is NOT one of the history-managed config files', async () => {
  ...
  assert.notEqual(HISTORY_PATH, SECRETS_PATH);
  assert.equal(existsSync(`${SECRETS_PATH}.bak`), false);
});
...
ok('a pre-blocks set reads as exactly ONE implicit Plex block', () => {
  ...
  // Every `b[0]!` below asserts the `b.length` check on the line above it.
