...
const RK = '9001';
/** One leaf as this fake Plex serves it — the subset of a Plex episode nextEpisode reads. */
interface FakeLeaf {
  type: string;
  ratingKey: string;
  parentIndex: number;
...
const WATCHED: Record<string, Set<number>> = { 'admin-token': new Set([1, 2]), 'kid-token': new Set() };
...
const leavesFor = (tok: string) => [1, 2, 3].map((i) => ep(i, WATCHED[tok]?.has(i)));
const hits: Record<string, number> = { 'admin-token': 0, 'kid-token': 0 };

const srv = createServer((req, res) => {
  res.setHeader('content-type', 'application/json');
  const url = (req.url || '').split('?')[0];
  // A header can arrive as string[]; every request here sends one value, and String() is
  // what the object keys below are indexed by either way.
