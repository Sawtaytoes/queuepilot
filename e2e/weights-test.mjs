...
// stub client the way resume-in-queue-test.mjs does.
//
...
// --- the interleave -------------------------------------------------------------------------
const bucket = (name, n, weight) => ({
  name,
  weight,
  episodes: Array.from({ length: n }, (_, i) => `${name}${i + 1}`),
});
const run = (buckets, length) => weightedInterleave(buckets, (b) => b.episodes, length);
const shows = (out) => out.map((x) => x[0]).join('');
...
const tally = (out, ch) => out.filter((x) => x[0] === ch).length;
...
function legacyRoundRobin(order, length) {
  const cursors = new Map(order.map((s) => [s.name, 0]));
  const queue = [];
...
let cursor = 0;
const seeded = { random: () => seq[cursor++ % seq.length], shuffle: () => {} };
let heavyFirst = 0;
const TRIALS = 200;
for (let t = 0; t < TRIALS; t += 1) {
  const members = [{ n: 'light', weight: 1 }, { n: 'heavy', weight: 5 }, { n: 'light2', weight: 1 }];
...
const roster = [{ n: 'a', weight: 1 }, { n: 'b', weight: 4 }, { n: 'c', weight: 2 }, { n: 'd' }];
