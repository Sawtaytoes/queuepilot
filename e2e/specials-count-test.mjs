...
//
// Run:  node e2e/specials-count-test.mjs   (from the repo root; non-zero exit on failure)
import { promises as fs } from 'node:fs';

process.env.CACHE_PATH = '/tmp/specials-count-cache.sqlite';
for (const f of [process.env.CACHE_PATH, `${process.env.CACHE_PATH}-wal`, `${process.env.CACHE_PATH}-shm`]) {
...
const s1 = (rk, i, watched) => ({ ratingKey: rk, parentIndex: 1, index: i, title: `Ep ${i}`, duration: DUR, viewCount: watched ? 1 : 0 });
// Season-0 items, classified by index range: 1–99 special, 200–299 trailer, 300–399 OP/ED, 400+ other.
const s0 = (rk, index, title, watched = false) => ({ ratingKey: rk, parentIndex: 0, index, title, duration: DUR, viewCount: watched ? 1 : 0 });
...
for (const [rk, eps] of [['100', s1Leaves], ['200', s2Leaves], ['300', s3Leaves], ['400', s4Leaves]]) {
