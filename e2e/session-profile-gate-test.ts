...
// e2e/profile-gate-test.ts (waitForProfile / setForProfile against a synthetic PMS log).
//
// Run:  server/node_modules/.bin/tsx e2e/session-profile-gate-test.ts   (from the repo root; non-zero on failure)
...
import type { SessionStartPayload } from '../server/src/types.js';
import type { SessionStateExtra } from '../server/src/session.js';

/**
 * The harness control surface, typed.
 *
 * `stubs/session-harness.mjs` stays hand-written JavaScript on purpose — it is `registerHooks`
 * machinery that has to run before anything is transpiled — so under `allowJs`/`checkJs: false`
 * its `SESSION_CTL` is INFERRED from an initializer of nulls and empty arrays: `profileTitle`
 * comes out as the type `null`, `plays`/`drives` as `never[]`. Assigning a real profile name to
 * it is then an error about the fixture rather than about the code under test. This declares the
 * shape the stub actually carries at runtime; it is a view of the same object, not a copy.
 */
...
async function run(payload: SessionStartPayload, signedInAs: string | null = null) {
  resetSession();
  STATES.length = 0;
