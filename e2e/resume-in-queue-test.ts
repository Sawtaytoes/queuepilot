...
//
// Run:  server/node_modules/.bin/tsx e2e/resume-in-queue-test.ts   (repo root; non-zero on failure)
...
import type { Device, PlexClient, PlexMediaContainer } from '../server/src/types.js';
import type { EntryDescriptor } from '../server/src/engine/resolve.js';

const SCRATCH = mkdtempSync(nodePath.join(tmpdir(), 'resume-'));
// Local consts as well as the env assignment: `process.env.X` reads back as
// `string | undefined`, and writeFileSync wants the path it was just given.
...
const VIEW_STATE: Record<string, [viewOffset: number, viewCount: number]> = {
...
let RESOLVE: Record<string, string[]> = {};
const leafClient: PlexClient = {
    ...
    if (leaves) {
      ...
      return {
        Metadata: rks.map((rk, i) => ({
          ...
          duration: 1000,
          type: 'episode',
          viewCount: VIEW_STATE[rk]?.[1] ?? 0,
          viewOffset: VIEW_STATE[rk]?.[0] ?? 0,
        })),
      };
    }
...
// The descriptor fields nextQueue actually reads. `as EntryDescriptor` documents that the
// rest (year/guid/weight/raw) is deliberately absent — as it was before the conversion, so
// the resolver is exercised on exactly the same input.
const entriesFor = (keys: string[], episodes: number | null = null): EntryDescriptor[] =>
  keys.map((k) => ({
...
RESOLVE = {};
let res = await resolve.nextQueue(leafClient, 'bob', CFG, entriesFor(['inprog']), new Set<string>(), null);
...
res = await resolve.nextQueue(leafClient, 'bob', CFG, entriesFor(['showX'], 2), new Set<string>(), null);
...
RESOLVE = { showX: ['inprog', 'fresh'] };
res = await resolve.nextQueue(leafClient, 'bob', CFG, entriesFor(['showX']), new Set<string>(), null);
...
RESOLVE = {};
res = await resolve.nextQueue(leafClient, 'bob', CFG, entriesFor(['fresh']), new Set<string>(), null);
...
res = await resolve.nextQueue(leafClient, 'bob', CFG, entriesFor(['rewound']), new Set<string>(), null);
...
`;
// The stub module below is a DATA-URL module — a separate module instance — so it can only
// reach this array through globalThis. The cast rather than a `declare global`: binding-token-
// test.ts already declares that same global as its own richer `{url, headers}[]`, and the two
// harnesses never share a process, so each keeps its own shape locally instead of the two
// declarations fighting in one program.
(globalThis as unknown as { __PLEX_CALLS: string[] }).__PLEX_CALLS = CALLS;
...
const playMediaQuery = (): Record<string, string> => {
  const call = CALLS.find((c) => c.includes('playMedia')) || '';
  const q = call.includes('?') ? call.slice(call.indexOf('?') + 1) : '';
  return Object.fromEntries(new URLSearchParams(q));
};

// The two fields playback.js reads off a device on the client path. The rest of `Device`
// (id/name/machineIdentifier/default) is deliberately absent — as it was before the
// conversion — so this stays the same input the playMedia builder saw.
const CLIENT_DEVICE = { mode: 'client', uri: 'http://shield.invalid:32500' } as Device;

// Resume: a non-zero offset is passed straight through as Companion's `offset` (ms).
CALLS.length = 0;
await playback.playRatingKeys(['inprog', 'fresh'], {
  setName: 'bob', device: CLIENT_DEVICE, offset: 45000,
});
...
await playback.playRatingKeys(['fresh'], {
  setName: 'bob', device: CLIENT_DEVICE, offset: 0,
});
