...
//
// Run:  node e2e/kavita-covers-test.mjs   (from the repo root; non-zero on failure)
...
const kavita = createServer((req, res) => {
  ...
  if (url.pathname === '/api/Series/series-detail') {
    return json(DETAIL[url.searchParams.get('seriesId')] ?? { chapters: [], specials: [] });
  }
...
await new Promise((r) => kavita.listen(0, '127.0.0.1', r));
const KAVITA_URL = `http://127.0.0.1:${kavita.address().port}`;
...
const api = (p, opts) => fetch(`http://localhost:${PORT}/api${p}`, opts);
...
try {
  ...
  const porter = results.find((r) => r.title === 'Dungeon Porter');
  ok(
    `every result carries a cover URL (${porter?.cover})`,
    results.every((r) => r.cover === `/api/providers/kavita/cover/${r.ratingKey}`),
  );
  ...
  const dp = items.find((i) => i.key.includes('599') || i.title === 'Dungeon Porter');
  ...
  const fin = items.find((i) => i.title === 'Finished Series');
  ok('a fully-read series resolves with nothing next', fin?.resolved === true && fin?.nextEp === null);

  const gone = items.find((i) => i.title === 'Deleted From Kavita');
  ok('a vanished series degrades to its stored title', gone?.resolved === false && gone?.ratingKey === null);
  ok('…and does not take the other entries with it', items.filter((i) => i.resolved).length === 2);
