// The `pick.ts` <-> `SelectListbox` contract. Browser, but NO Plex.
//
...
// native <select> left, so `e2e/pick.ts` replaced `selectOption(sel, value)`
...
// use `pick.ts` all live in the `PLEX_TOKEN`-gated step, and ci.yml's own
...
//   - Every option label carries `[data-value]` — what `pick.ts` picks by.
...
const ok = (name: string, isPass: boolean, detail = '') => {
...
const stopVite = () => {
  ...
  isStopped = true;
  try {
    // Negative pid = the whole group, so vite's own children go too. `pid` is
    // `undefined` only when the spawn itself failed, in which case there is nothing to
    // signal and `waitForServer` below is what reports it.
...
let viteExit: number | 'signal' | null = null;
...
console.log(process.exitCode ? '\npick.ts contract BROKEN' : '\npick.ts contract holds');
