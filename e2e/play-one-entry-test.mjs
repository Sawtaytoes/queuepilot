...
//
// Run:  node e2e/play-one-entry-test.mjs   (from the repo root; non-zero on failure)
...
ok('entryKey matches the key the grid holds',
  resolve.entryKey('gamma') === 'title:gamma', resolve.entryKey('gamma'));
...
ok('entryKey agrees with queues.js for a ratingKey entry',
  resolve.entryKey({ ratingKey: 361504 }) === 'rk:361504', resolve.entryKey({ ratingKey: 361504 }));
...
ok('`only`: the named entry plays instead of the head',
  res.play.length && res.play[0].ratingKey === 'gamma-e1',
  JSON.stringify(res.play.map((i) => i.ratingKey)));
...
ok('`only`: plays the entry\'s next UNWATCHED episode, not its first',
  res.play.length && res.play[0].ratingKey === 'gamma-e2',
  JSON.stringify(res.play.map((i) => i.ratingKey)));
...
// request error, not a field to ignore. Asserted as the shape server.js branches on, since
// importing server.js would boot a listener and an MQTT client.
const rejects = (s) => Boolean(s.source !== 'queue');
...
const payloadFor = (setId, kind, target, profile, only) => {
