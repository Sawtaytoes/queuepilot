...
/**
 * The stubs' shared control surface, typed.
 *
 * `stubs/control.mjs` stays hand-written JavaScript — it is the mutable object the
 * `registerHooks` stubs close over — so under `allowJs`/`checkJs: false` it is INFERRED from
 * its initializer: `lastSeen.title` comes out as the type `null` and `playResults`/`onSwitch`
 * as `never[]`/`null`, which makes every scenario setup below an error about the fixture rather
 * than about the driver. This declares what the stub actually carries at runtime; it is a view
 * of the same object, not a copy.
 */
interface DriverCtl {
...
function cancelFlag(): CancelFlag & { set: () => void } {
  let set = false;
  return { set: () => { set = true; }, isSet: () => set };
}

/** The FSM branches on `device.mode` and nothing else here (the stubbed playback never reads
 * the rest of the row), so the harness hands it a deliberately partial device. Widened at the
...
// --------------------------------------------------------------------------- //
reset();
res = await drive({ device: asDevice('cast') });
...
// The preceding `drive()` always records a play, so the `!` asserts the line above it.
