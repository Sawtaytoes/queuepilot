...
//
// Run:  node e2e/device-registry-test.mjs     (from the repo root; exits non-zero on failure)
...
globalThis.fetch = async (url) => {
  ...
  if (outcome && outcome.__status) {
    return { ok: false, status: outcome.__status, text: async () => '' };
  }
...
const SERVER_ROW = { name: 'nas', clientIdentifier: 'pms-machine-id', provides: 'server' };
