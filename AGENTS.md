...
- **A class name is not a style — never copy one into a view it was not scoped for.** Most
  ...
  un-scoping rules. ⚠️ **THE NUMBER IS A FLOOR, NOT A CONSTANT — do not read a difference as a
  regression on its own.** Measured 2026-09-08 over four runs of the same commit: **11, 11, 11,
  13**. The floor of 11 is eight state or ancestor classes (`.editable`, `.modalbtns`,
  `.name-editable`, `.play-view`, `.queue-view` on three bodies, `.rules-shelf`), two of
  Tailwind's own `peer` primitive, and one **`.playbtn`**, which is deliberately a DOM handle
  carrying no rule — `PlayMenu`'s outside-click handler asks `t.closest(".playbtn")`, so the
  class has to exist and must not paint. **`.addanother` is a second one of those** — the queue
  search row's duplicate door, which `SearchDropdown` names in `ignoreSelector` so pressing it
  does not also fire the row's "jump to the one already here". The audit never sees it, because
  it visits routes and the results list is closed on all of them. The two extra pairs in the
  fourth run are `.badge` and `.unresolved` on `/channels`, and they appear only when that route
  paints its data before the audit reads it — the same run reported `flagged=112` there against
  `flagged=2` in its own earlier pass. So compare the CLASS LIST, not the total: a class that
  was never on it is the finding. A new class here needs a reason written down at this line.
  (The earlier figure of 18 at this line was never reproduced and is withdrawn.) ⚠️ Do not trust a zero from it — its first
